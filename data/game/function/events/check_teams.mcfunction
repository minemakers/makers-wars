...
execute if score #count var matches ..1 as @a[gamemode=!spectator] unless score @s confirm matches 0.. run tellraw @s [{"storage":"game:lang","nbt":"dialog.stop.text","color":"red","interpret":true},{"text":" [","color":"gold","extra":[{"storage":"game:lang","nbt":"dialog.stop.actions.1.name","interpret":true},{"text":"]"}],"hover_event":{"action":"show_text","value":[{"storage":"game:lang","nbt":"dialog.stop.actions.1.hover","interpret":true}]},"click_event":{"action":"run_command","command":"/trigger confirm set 1"}}]
